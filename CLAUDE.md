...
- `src/main/` — main process. `ck3.ts` has install/mod detection (Steam via registry + libraryfolders.vdf, `.mod` descriptor parsing). `pdx.ts` is the shared Paradox-script scanner: `scanBlocks` returns exact byte spans over RAW text (comment/quote-aware) so edits can splice blocks while leaving the rest of a file byte-identical — `saveCharacter` in `characters.ts` relies on this; a no-op save must round-trip byte-for-byte. `lineEditor.ts` does the writing: `setScalar` for single-valued keys, `setRepeatedScalar` for repeating ones (`trait`, `doctrine`, `holy_site`) as a multiset diff so surviving lines keep their comments and order, and `setBlockList`/`setBlockBody` for block-valued ones (`traditions`, `parents`, the `*_gfx` bundles), which keep a block's existing shape — single-line stays single-line, multi-line keeps its inner indentation. `scriptFile.ts` is the counterpart for *adding* a top-level block (`appendBlock`, key/file-name validation), shared by `createCharacter` and `createDynasty`/`createHouse`. `refdata.ts` collects culture/faith/trait ids by layering mod files over game files (mod file with same relative path wins; `replace_path` folders exclude game files entirely). `religions.ts` reads faiths nested two levels deep (`<religion> = { faiths = { <faith> = { … } } }`) and composes the offsets to splice a faith's body in place. `cultures.ts` reads `common/culture/cultures` plus the pillar/tradition/name-list/ethnicity option lists, and resolves culture colours through `common/named_colors`. `fonts.ts` reads `fonts/fonts.font` — the file naming CK3's fonts and binding each to font files — and resolves `StandardGameFont`/`TitleFont` to data URLs the renderer installs as web fonts. `settings.ts` persists JSON to `%APPDATA%/ck3-tools/settings.json`.
...
- Remembered rows — every editor keeps favorites, recents and unsaved drafts of its own rows, per mod, in settings.json under a tool key (`characters`, `dynasties`, `cultures`, `faiths`, `religions`, `titles`). `useEntryHistory(tool)` (`hooks/`) reads and writes all three; `EntryHistoryBar` draws them as the band of ReferenceBadge chips above a list, with `resolve` mapping a stored ref onto the current scan (a fresher name, or null to hide a row the mod no longer loads) and `visual` giving the chip its leading square. `FavoriteToggle` is the star a list row carries. An `EntryRef` is `{ id, name, scope? }`; `scope` is the coordinate an id needs when it doesn't name a row on its own — a character's history file, `dynasty` vs `house` — and `entryKey` in `@shared/entries` is what keys all three stores (trimmed, lowercased, scope-prefixed).
...
- Cultures: a culture carries no `name` scalar — its display name is the localization of the id itself, so the editor shows it read-only (same boundary as faiths). `color` comes in five shapes (a `common/named_colors` name, `rgb`, `hsv`, `hsv360`, or a bare triple that is 0-255 when every component is an integer and 0-1 otherwise); all five resolve to a swatch and an edit is written back in whichever spelling the file already used, a named colour becoming `rgb`. Because rewriting a block normalizes its layout, `saveCulture` compares every field against the file first and writes only the ones that actually changed — that is what keeps a no-op save byte-identical. Resolving the named-colour palette is part of that: without it a `color = english` reads as "no colour" and every save would rewrite it.
- Faiths and religions: a faith's `color` comes in four shapes (`{ r g b }` in 0-1 floats or 0-255 ints, `hsv`/`hsv360`, or a `common/named_colors` name). All four resolve to a swatch; only the plain triples are rewritten, in whichever numeric style the line already used, and only when the value actually changed. Doctrines are a flat repeating `doctrine =` list in the file but mutually exclusive picks per `doctrine_group_types` group in the UI, and a faith inherits its religion's pick for any group it leaves unset. Faith/religion display names come from localization keyed on the id and are read-only here — the app doesn't write .yml files. Faiths and religions are separate sidebar tools (`/faiths`, `/religions`); a deep-linked id that turns out to belong to the other editor is handed over with a replace-navigation, so cross-references always land somewhere useful.
...
- Mod fonts: the `useModFonts` setting (default on) dresses the UI in the selected mod's own CK3 fonts — `StandardGameFont` for body text, `TitleFont` for headings. Resolution layers like any other content: a mod's `fonts/fonts.font` replaces the game's outright, and each font file it names is looked up mod-first, so a mod can redress the app either by rebinding names or by dropping a replacement file next to the game's. When the mod contributes neither, `getModFonts` returns null and the app keeps Inter. The renderer installs the faces by appending a stylesheet that redefines the `--sans` / `--heading` tokens; those live in plain `:root` precisely so the `@theme inline` block can point the `font-sans`/`font-heading` utilities at them — inlining the values there instead would make them unoverridable.
