...
Electron-based suite of editing tools for Crusader Kings III. Built: Character Editor, Dynasty & House Editor, Faith Editor. Planned: Culture Editor (placeholder exists; not yet implemented).
...
- `src/main/` — main process. `ck3.ts` has install/mod detection (Steam via registry + libraryfolders.vdf, `.mod` descriptor parsing). `pdx.ts` is the shared Paradox-script scanner: `scanBlocks` returns exact byte spans over RAW text (comment/quote-aware) so edits can splice blocks while leaving the rest of a file byte-identical — `saveCharacter` in `characters.ts` relies on this; a no-op save must round-trip byte-for-byte. `lineEditor.ts` does the writing: `setScalar` for single-valued keys and `setRepeatedScalar` for repeating ones (`trait`, `doctrine`, `holy_site`), the latter as a multiset diff so surviving lines keep their comments and order. `scriptFile.ts` is the counterpart for *adding* a top-level block (`appendBlock`, key/file-name validation), shared by `createCharacter` and `createDynasty`/`createHouse`. `refdata.ts` collects culture/faith/trait ids by layering mod files over game files (mod file with same relative path wins; `replace_path` folders exclude game files entirely). `religions.ts` reads faiths nested two levels deep (`<religion> = { faiths = { <faith> = { … } } }`) and composes the offsets to splice a faith's body in place. `settings.ts` persists JSON to `%APPDATA%/ck3-tools/settings.json`.
