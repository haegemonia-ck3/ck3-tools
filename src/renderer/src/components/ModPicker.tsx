...
interface ModPickerProps {
  /** Render only the mod list, without the Card chrome (for use inside a dialog). */
  plain?: boolean
  onSelect?: (file: string) => void
}

export default function ModPicker({ plain = false, onSelect }: ModPickerProps): React.JSX.Element {
  ...
  const list = (
    <>
      ...
      <RadioGroup
        ...
        onValueChange={(file) => {
          void updateSettings({ selectedModFile: file })
          onSelect?.(file)
        }}
      >
        {mods.map((mod) => (
